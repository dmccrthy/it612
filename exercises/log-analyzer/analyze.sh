...
# use -F here to set awk field seperator (so we can grab just the username)
grep "session created for user=" server.log | awk -F = '{print $2}' | sort | uniq -c | sort -rn
echo ""
...
echo "Report generated: $(date +"%m/%d/%Y")"